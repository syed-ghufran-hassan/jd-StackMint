...
(define-public (buy-nft (token-id uint))
  (let ((listing (unwrap! (map-get? listings token-id) ERR_NOT_LISTED)))
    ...
    (let ((price (get price listing))
          ...
          (metadata (contract-call? .stacksmint-nft-v2-1 get-token-metadata token-id))
      ...
      (let ((marketplace-fee (/ (* price u25) u1000))
        ...
        ;; Transfer NFT
        (try! (contract-call? .stacksmint-nft-v2-1 transfer token-id seller tx-sender))
        ;; Update stats
  ...
  (let ((offer (unwrap! (map-get? offers { token-id: token-id, offerer: offerer }) ERR_OFFER_NOT_FOUND)))
    ...
    (let ((amount (get amount offer))
      ...
      (let ((marketplace-fee (/ (* amount u25) u1000))
        ...
        ;; Transfer NFT
        (try! (contract-call? .stacksmint-nft-v2-1 transfer token-id tx-sender offerer))
        ;; Update stats
    ...
    (match (get highest-bidder auction)
      winner (let (
        ...
        (let ((marketplace-fee (/ (* final-price u25) u1000))
          ...
          ;; Transfer NFT
          (try! (contract-call? .stacksmint-nft-v2-1 transfer token-id seller winner))
          ;; Update stats
...
(define-private (transfer-bundle-nft (token-id uint) (prev (response principal uint)))
  (match prev
    seller (match (contract-call? .stacksmint-nft-v2-1 transfer token-id seller tx-sender)
      result (ok seller)
      err prev)
    err prev))
