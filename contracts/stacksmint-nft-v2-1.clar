...
(define-non-fungible-token stacksmint-nft-v2-1 uint)
...
(define-private (is-owner-or-approved (token-id uint) (sender principal))
  (let ((owner (unwrap! (nft-get-owner? stacksmint-nft-v2-1 token-id) false)))
    (or 
      (is-eq sender owner)
...
(define-public (mint-full 
  ...
  (begin
    ...
    (let ((token-id (+ (var-get last-token-id) u1))
      ...
      (try! (nft-mint? stacksmint-nft-v2-1 token-id tx-sender))
...
(define-public (mint-as-delegate (owner principal) (uri (string-ascii 256)))
  (begin
    ...
    (let ((token-id (+ (var-get last-token-id) u1)))
      (try! (contract-call? .stacksmint-treasury-v2-1 collect-fee))
      (try! (nft-mint? stacksmint-nft-v2-1 token-id owner))
      (map-set token-uris token-id uri)
      (map-set token-metadata token-id {
        creator: owner,
        royalty-percent: u0,
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? stacksmint-nft-v2-1 token-id sender recipient)))
...
(define-public (approve (spender principal) (token-id uint))
  (let ((owner (unwrap! (nft-get-owner? stacksmint-nft-v2-1 token-id) ERR_NOT_FOUND)))
...
(define-public (burn (token-id uint))
  (begin
    (asserts! (is-owner-or-approved token-id tx-sender) ERR_NOT_AUTHORIZED)
    (try! (nft-burn? stacksmint-nft-v2-1 token-id tx-sender))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacksmint-nft-v2-1 token-id)))
