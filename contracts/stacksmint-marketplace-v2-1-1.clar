...
(define-public (buy-nft (token-id uint))
  (let ((listing (unwrap! (map-get? listings token-id) ERR_NOT_LISTED)))
    ...
    (let ((price (get price listing))
          ...
          (metadata (contract-call? .stacksmint-nft-v2-1-1 get-token-metadata token-id))
      ...
      (try! (contract-call? .stacksmint-treasury-v2-1 collect-marketplace-fee price creator royalty-percent))
      ...
      (let ((marketplace-fee (/ (* price u25) u1000))
        ...
        ;; Transfer NFT
        (try! (contract-call? .stacksmint-nft-v2-1-1 transfer token-id seller tx-sender))
        ;; Update stats
...
(define-public (accept-offer (token-id uint) (offerer principal))
  (let ((offer (unwrap! (map-get? offers { token-id: token-id, offerer: offerer }) ERR_OFFER_NOT_FOUND)))
    ...
    (let ((amount (get amount offer))
      ...
      (try! (as-contract (contract-call? .stacksmint-treasury-v2-1 collect-marketplace-fee amount creator royalty-percent)))
      ...
      (let ((marketplace-fee (/ (* amount u25) u1000))
        ...
        ;; Transfer NFT
        (try! (contract-call? .stacksmint-nft-v2-1-1 transfer token-id tx-sender offerer))
        ;; Update stats
  ...
  (let ((auction (unwrap! (map-get? auctions auction-id) ERR_NOT_LISTED)))
    ...
    (match (get highest-bidder auction)
      winner (let (
          ...
          (creator (match metadata data (get creator data) seller)))
        ;; Collect fees
        (try! (as-contract (contract-call? .stacksmint-treasury-v2-1 collect-marketplace-fee final-price creator royalty-percent)))
        ;; Calculate net payment
        (let ((marketplace-fee (/ (* final-price u25) u1000))
          ...
          ;; Transfer NFT
          (try! (contract-call? .stacksmint-nft-v2-1-1 transfer token-id seller winner))
          ;; Update stats
...
(define-public (buy-bundle (bundle-id uint))
  (let ((bundle (unwrap! (map-get? bundles bundle-id) ERR_NOT_LISTED)))
    ...
    (let ((price (get price bundle))
      ...
      (try! (contract-call? .stacksmint-treasury-v2-1 collect-marketplace-fee price seller u0))
      (let ((marketplace-fee (/ (* price u25) u1000))
...
(define-private (transfer-bundle-nft (token-id uint) (prev (response principal uint)))
  (match prev
    seller (match (contract-call? .stacksmint-nft-v2-1-1 transfer token-id seller tx-sender)
      result (ok seller)
      err prev)
    err prev))
